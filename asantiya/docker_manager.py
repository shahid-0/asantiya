...
class DockerManager:
    ...
    def connect(self) -> docker.DockerClient:
        ...
        try:
            if not self.config.builder.local:
                if not self.config.builder.remote:
                    raise DockerError("Remote Docker URL not configured")
                self.docker_client = self.docker.DockerClient(
                    base_url=self.config.builder.remote
                )
            else:
            ...
            error_msg = f"Unexpected error connecting to Docker: {str(e)}"
            _logger.error(error_msg)
            raise DockerError(error_msg) from e

    def check_docker_version(self) -> str:
        """Check Docker version with proper error handling."""
        if not self.docker_client:
            error_msg = "Docker client is not connected. Please call connect() first."
            _logger.error(error_msg)
        ...
        try:
            ...
            error_msg = f"Failed to retrieve Docker version: {str(e)}"
            _logger.error(error_msg)
            raise DockerError(error_msg) from e
        except Exception as e:
            error_msg = f"Unexpected error while checking Docker version: {str(e)}"
            _logger.error(error_msg)
            raise DockerError(error_msg) from e
    ...
    def _get_service_name(self, accessory: AccessoryConfig, service_name: str) -> str:
        ...
        service_name = (
            ...
            else "asantiya-{name}".format(name=service_name)
        )
        return service_name
    ...
    def _find_accessory_by_name(self, name: str):
        ...
        for accessory in self.config.accessories.keys():
            if name == self._get_service_name(
                self.config.accessories[accessory], accessory
            ):
                _accessory = self._get_service_name(
                    self.config.accessories[accessory], accessory
                )
                break
    ...
    def delete_image(
        self, image_name: str, force: bool = False, prune: bool = True
    ) -> bool:
        """
        ...
        Raises:
            DockerError: If deletion fails for non-404 reasons
        """
        if not image_name:
            raise DockerError("Image name cannot be empty")

        if not self.docker_client:
            raise DockerError("Docker client not connected")
        ...
        try:
            ...
            self.docker_client.images.remove(image.id, force=force, noprune=not prune)
            ...
            error_msg = f"Unexpected error deleting image {image_name}: {str(e)}"
            _logger.error(error_msg)
            raise DockerError(error_msg) from e

    def delete_images(
        self, image_names: List[str], force: bool = False, stop_on_error: bool = False
    ...
    def pull_images(self, images: Annotated[List[str], "List of docker images"]):
        if (
            ...
            or not all(isinstance(img, str) and img.strip() for img in images)
        ):
            raise ValueError(
    ...
    def stop_accessory(
        self, name: str, force: bool = False, raise_errors: bool = False
    ) -> None:
        """
    ...
    def list_configured_containers(self) -> None:
        ...
        try:
            ...
            rows = self._get_container_table_rows(
                self.config.accessories, all_containers
            )
    ...
    def _get_container_table_rows(
        self, configs: Dict[str, AccessoryConfig], all_containers
    ) -> List[List[str]]:
        rows = []
    ...
    def show_accessory_logs(
        self, name: str, follow: bool = False, tail: int = 100, timestamps: bool = False
    ) -> None:
        """
        ...
        try:
            ...
            print(
                f"=== Showing logs for {name} ({container.image.tags[0] if container.image.tags else 'no image'}) ==="
            )

            # Get and display logs
            logs = container.logs(
                stream=follow, follow=follow, tail=str(tail), timestamps=timestamps
            )
    ...
    def reboot_single_accessory(
        self, accessory_name: str, force: bool = False
    ) -> Union[bool, str]:
        """
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to remove {container_name}: {e.explanation}"
                )

            # 2. Create new container
            new_container = self.create_accessory(accessory, accessory_name)
            _logger.info(
                f"Successfully rebooted {accessory_name} (new container: {new_container.id[:12]})"
            )
            return True
    ...
    def reboot_all_accessories(self, force: bool = False) -> None:
        """
        ...
        for accessory_name in self.config.accessories.keys():
            self.reboot_single_accessory(accessory_name, force=force)
